...
function btn_player(btn_id,pidx)
	-- multiplayer: player 1 = WASD/SPACE/E (PICO-8 player 1),
	-- player 2 = ARROWS/Z/X (PICO-8 player 0)
	if multiplayer_mode then
		if pidx==1 then return btn(btn_id,1) else return btn(btn_id,0) end
	end
	-- single player: selected control scheme
	result = false
	if control_scheme != 2 then result = result or btn(btn_id,0) end
	if control_scheme != 1 then result = result or btn(btn_id,1) end
	return result
end

function btnp_player(btn_id,pidx)
	if multiplayer_mode then
		if pidx==1 then return btnp(btn_id,1) else return btnp(btn_id,0) end
	end
	local result = false
	if control_scheme != 2 then result = result or btnp(btn_id,0) end
	if control_scheme != 1 then result = result or btnp(btn_id,1) end
	return result
end

function btn_mapped(btn_id)
	return btn_player(btn_id,active_player_index)
end

function btnp_mapped(btn_id)
	return btnp_player(btn_id,active_player_index)
end
...
function update_control_tutorial()
	...
	if tutorial_step == 1 then
		if multiplayer_mode then text = "p1: wasd  p2: arrows"
		elseif control_scheme == 0 then text = "press WASD or ARROWS to move"
		elseif control_scheme == 1 then text = "press ARROWS to move"
		else text = "press WASD to move" end
	elseif tutorial_step == 2 then
		if multiplayer_mode then text = "p1: space  p2: z to jump"
		elseif control_scheme == 0 then text = "press SPACE or Z to jump"
		elseif control_scheme == 1 then text = "press Z to jump"
		else text = "press SPACE to jump" end
	elseif tutorial_step == 3 then
		if multiplayer_mode then text = "p1: e  p2: x to interact"
		elseif control_scheme == 0 then text = "press E or X to interact"
...
function _update()
	if game_state==0 then
		...
		-- navegação do menu principal: new game / multiplayer / credits / controls
		if got_coin_timer < 4 then
			if btnp_mapped(2) then 
				got_coin_timer=(got_coin_timer-1+4)%4
			elseif btnp_mapped(3) then 
				got_coin_timer=(got_coin_timer+1)%4
			end
		...
		-- tela de credits (got_coin_timer==5)
		if got_coin_timer==5 then
			if btnp_mapped(5) then got_coin_timer=2 end
			return
		end
		...
		if got_coin_timer==4 then
			...
			if btnp_mapped(4) then 
				...
				got_coin_timer=3  -- Z volta ao menu
				return
			end
			if btnp_mapped(5) then 
				got_coin_timer=3  -- X também volta sem salvar
			end
			return
		end
	end
...
function _draw()
	...
	if game_state==0 then
		...
		if got_coin_timer==0 then s1=7
		elseif got_coin_timer==1 then s2=7
		elseif got_coin_timer==2 then s3=7
		else s4=7 end
	...
	local function draw_current_player(pidx, invert)
		...
		if not pp or respawn_timer>0 then return end
		if got_coin_timer > 0 then return end
		if invert then
			for c=1,15 do pal(c,15-c,1) end
		end
...
function make_player(x,y)
	return {
		x=x, y=y, xv=0, yv=0, face_l=true, frame=1, frame_delay=2,
		on_ground=false, was_ground=false, on_wall=false, wall_side=0,
		platform=nil, bubble=nil, fly_timer=0, aircontrol_timer=0, no_bubble_timer=0
	}
end
...
function select_player(pidx)
	active_player_index=pidx
	player=players[pidx]
	jump_pressed=jump_pressed_by_player[pidx]
end

function save_active_player()
	players[active_player_index]=player
	jump_pressed_by_player[active_player_index]=jump_pressed
end
...
function update_player_slot(pidx)
	select_player(pidx)
	update_input()
	move_player()
	animate_player()
