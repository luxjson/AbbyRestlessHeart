...
function _update()
	if game_state==0 then
		...
		if got_coin_timer==2 then
			if btnp(5) then got_coin_timer=1 end
			return
		end
		if btnp(2) or btnp(3) then got_coin_timer=1-got_coin_timer end
		if btnp(4) then
			if got_coin_timer==0 then respawn_timer=-12 else got_coin_timer=2 end
		end
		return
	end
...
function _draw()
	...
	if game_state==0 then
		...
		for x=4,124,12 do for y=8,92,12 do print("♥",x,y,1) end end
		print("Abby's",52,48,7)
		print("Restless",48,58,7)
		print("Heart",54,68,7)
		rectfill(0,101,127,127,8)
		local t=got_coin_timer==0 and "NEW GAME" or "CREDITS"
		print("⬆️",38,112,7)
		print(t,got_coin_timer==0 and 48 or 50,112,7)
		print("⬇️",82,112,7)
		return
	end
